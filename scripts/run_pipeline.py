...
def main(args):
    ...
    with mlflow.start_run():
        ...
        precision = float(precision_score(y_test, y_pred))    # Of predicted churners, how many actually churned?
        recall = float(recall_score(y_test, y_pred))          # Of actual churners, how many did we catch?
        f1 = float(f1_score(y_test, y_pred))                  # Harmonic mean of precision and recall
        roc_auc = float(roc_auc_score(y_test, proba))         # Area under ROC curve (threshold-independent)
...
"""
# Use this below to run the pipeline in PowerShell:
